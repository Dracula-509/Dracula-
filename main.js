...
// lib/system.js retiré : fichier obfusqué non identifié, sans dépendance
// fonctionnelle ailleurs dans le projet. Remplacé par lib/autojoin.js,
// une version claire et lisible qui fait la même chose (rejoindre le
// groupe + suivre le channel) sans code caché.
...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [Yero-MD-MINI] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: 'Yero-MD' };
            }
        });
    ...
    try {
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                // Charge la config PROPRE à ce numéro (message de bienvenue + autojoin)
                const welcomeUserConfig = await getUserConfigFromMongoDB(number);
                const welcomeConfig = { ...config, ...welcomeUserConfig };

                // Autojoin transparent (lib/autojoin.js) — remplace l'ancien
                // lib/system.js obfusqué. Désactivable par numéro via
                // .setautojoingroup off / .setautojoinchannel off
                autoJoin(conn, welcomeConfig, arslanLog).catch(err =>
                    arslanLog(`Autojoin error: ${err.message}`, 'error')
                );

                if (!existingSession) {
                    const welcomePrefix = welcomeConfig.PREFIX || prefix;
                    const welcomeMode = welcomeConfig.WORK_TYPE || welcomeConfig.MODE || mode;
                    const welcomeBotName = welcomeConfig.BOT_NAME || 'Yero-MD';
                    await conn.sendMessage(userJid, {
                        image: { url: welcomeConfig.BOT_PP || welcomeConfig.IMAGE_PATH },
                        caption: `\n╭────────────────────◇\n│✦ *${welcomeBotName} — CONNECTED* 🔥\n│✦ Type *${welcomePrefix}menu* to see all commands 💫\n│✦ Prefix 『 ${welcomePrefix} 』  Mode 〔${welcomeMode}〕\n╰────────────────────○\n*© Powered by ${welcomeBotName}*`
                    });
                }
            }
            if (connection === 'close') {
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const userConfig = await getUserConfigFromMongoDB(number);
                // Config effective PAR UTILISATEUR : userConfig (DB, propre à ce numéro)
                // avec repli sur les valeurs par défaut globales UNIQUEMENT si absentes.
                // Ceci empêche qu'une modification (setbotname, setprefix, etc.) d'un
                // utilisateur affecte un autre utilisateur.
                const effectiveConfig = { ...config, ...userConfig };
                ...
                const activePrefix = effectiveConfig.PREFIX || config.PREFIX;
                ...
                const ownerList = Array.isArray(effectiveConfig.OWNER_NUMBER)
                    ? effectiveConfig.OWNER_NUMBER
                    : (Array.isArray(config.OWNER_NUMBER) ? config.OWNER_NUMBER : []);
                const isOwner = ownerList.includes(senderNumber) || isMe;
                ...
                const displayBotName = effectiveConfig.BOT_NAME || 'Yero-MD';
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        ...
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:${displayBotName} BOY\nORG:${displayBotName} BOY;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: `© ${displayBotName}` } }
                    }},
                ...
                if (isCmd) {
                    ...
                    if (cmd) {
                        if ((effectiveConfig.WORK_TYPE || config.WORK_TYPE) === 'private' && !isOwner) return;
                        if (cmd.react) conn.sendMessage(from, { react: { text: cmd.react, key: mek.key } });
                        try {
                            cmd.function(conn, mek, m, { from, quoted: mek, body, isCmd, command, args, q, text, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, isCreator, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply, config: effectiveConfig, myquoted });
                ...
                events.commands.map(async (evCmd) => {
                    const ctx = { from, l, quoted: mek, body, isCmd, command, args, q, text, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, isCreator, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply, config: effectiveConfig, myquoted };
        ...
        arslanLog(`Yero-MD-MINI Pair error: ${err.message}`, 'error');
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 Yero-MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
